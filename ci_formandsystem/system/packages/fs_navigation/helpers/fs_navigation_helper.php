...
function nav_check_url( $path )
{
	...
	if( substr($path, 0, 6) == 'https:' || substr($path, 0, 5) == 'http:' || substr($path, 0, 4) == 'www.' )
	{
		return $path;
	}
	...
	else
	{
		...
		if( is_array($languages) && ( !isset($path_lang[1]) || !in_array($path_lang[1], $languages) ) )
		{
			// no language -> add it
...
function navigation_item( &$item, $opt = array(NULL) )
{
	...
	// -----------------
	// return item
	return "<li".$class.$id.$item_data.">"."<a data-link-tracking='menu-item-".preg_replace('/[^0-9a-zA-Z_]/',"",$item['label'])."' class='".$link_class."menu-item-link' href=\"".$path."\"".$title.$target.$link_data.">".$opt['item_before'].$item['label'].(isset($shortcut) ? $shortcut : '').$opt['item_after']."</a>";
}
// --------------------------------------------------------------------
/**
