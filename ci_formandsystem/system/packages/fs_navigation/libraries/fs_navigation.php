...
class CI_FS_navigation {
	...
	function initialize($params = array(NULL))
	{
		$this->params = array_merge(array(
					'db' 			=> $this->CI->config->item('db_prefix').$this->CI->config->item('db_menu'),
					'status' 		=> '1',
			...
			$menu_items = db_select($this->params['db']);
			// loop through items
	...
	function tree( $params = array() )
	{
		...
		if( isset($menu) && isset($menu[$start]) )
		{
			...
			$count = 0;
			// get menu elements
			$elements = $this->_loop($menu, $start, $count, $params);
			// if menu elements exist build menu
			if($elements != null)
			{
				$output = "\n<".$params['list'].
					(!empty($params['id']) ? ' id="'.$params['id'].'"' : '').
					(!empty($params['class']) ? ' class="'.$params['class'].(isset($params['class_lvl_'.($count)]) ? ' '.$params['class_lvl_'.($count)] : '').'"' : '').
					(!empty($params['menu_id']) ? ' menu_id = "'.$params['menu_id'].'"' : '').
					">\n".$elements.'</'.$params['list'].">\n";
			}
		}
		// no menu given but show_empty true
		elseif( (!isset($menu) || !isset($menu[$start]) ) && isset($params['show_empty']) && $params['show_empty'] === TRUE)
		{
			// -----------------
			// show empty menu
			$output = "\n<".$params['list'].
